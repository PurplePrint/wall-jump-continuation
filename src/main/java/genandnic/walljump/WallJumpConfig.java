...
public class WallJumpConfig {
    ...
    @Comment("If 'true', player will not be able to jump during a hard fall to avoid fall damage.")
    public static boolean fairDoubleJump = true;

    @Comment("How fast player must move downwards for the double jump restriction to kick in. Must be a negative float number!")
//    public static float fairMotionY = -0.7F;
    public static float fairMotionY = -0.85F;

    @Comment("Double jumping will be allowed while falling if player is moving fast enough on the X or Z axis.")
    public static float fairMotionXZ = 0.26F;

    @Comment("Show icon when player is falling and can't double jump.")
    public static boolean showFallingHud = true;

    @Comment("X offset of the falling hud")
    public static int fallingHudX = -33;

    @Comment("Y offset of the falling hud")
    public static int fallingHudY = 11;
